...
export function DailyActionCenter({ actions }: DailyActionCenterProps) {
  return (
    <Panel
      title="Việc hôm nay"
      subtitle="Làm theo thứ tự ưu tiên — không bỏ sót khách và tiền"
    >
      {actions.length === 0 ? (
        <div className="flex items-start gap-3 rounded-lg bg-emerald-50 p-4 dark:bg-emerald-950/30">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-emerald-700/80 dark:text-emerald-400/80">
              Hôm nay bạn đã theo kịp khách. Xem insight doanh thu và danh sách sắp tới bên dưới.
            </p>
          </div>
        </div>
      ) : (
